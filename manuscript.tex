...
\begin{abstract}
Large language models pass examinations and solve complex problems, yet something appears missing. We propose a precise alternative to contested philosophical diagnoses: \textbf{life instantiates geometry; AI simulates geometry using information}.

\emph{Information} is what can be named, stored, and transmitted---bits, facts. \emph{Geometry} is the constrained manifold within which a system operates---the dimensionality of knowing, not just the quantity of known. A database has information; a living cell has knowing.

Current AI has vast information but minimal self-maintained geometry. Large language models simulate geometry using statistical patterns that approximate relational structure, but do not instantiate it: there is no underlying manifold whose dynamics are being maintained.

We formalize this distinction using results from dynamical systems theory. In systems with genuine geometry, independent complexity estimators---geometric (state-space occupancy) and spectral (oscillatory richness)---agree. We define \emph{match error} $\varepsilon$ measuring this agreement, predicting that living systems minimize $\varepsilon$ while artificial systems show systematic mismatch.

Biology serves as existence proof: life instantiates geometry through bidirectional cross-scale coupling. Dormancy proves geometry can exist without dynamics---structure persists when oscillations pause. Death is geometric collapse.

What would AI need to instantiate geometry? Not more parameters, but intrinsic temporal dynamics coupled to viability. The metabolic expense is not a cost of intelligence---it \emph{is} the intelligence.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{Geometry} is what cannot be copied---only instantiated. It is substrate-dependent.
\end{itemize}

You can copy a cell's genome (information). You cannot copy the cell's dynamical coherence (geometry). You would have to \emph{rebuild} it---construct a new substrate that instantiates the same geometric constraints through active maintenance.
...
We call this \textbf{vertical coupling}: downward regulation (coarse variables constrain fine dynamics) and upward closure (fine dynamics stabilize coarse variables), forming feedback across scales. Current AI systems are \emph{vertically flat}: they may have many layers, but higher representations do not actively maintain lower-level dynamics.
...
\begin{itemize}
    \item \textbf{Geometric estimators} measure state-space occupancy: correlation dimension, participation ratio, intrinsic dimension.
...
\end{equation}
where $D_{\text{geom}}$ is a geometric proxy (e.g., participation ratio), $D_{\text{spec}}$ is a spectral proxy (e.g., spectral entropy), and $\delta > 0$ is a small regularization constant.
...
Tardigrades in cryptobiosis, bacterial spores, frozen embryos, and seeds are not thermodynamically active \citep{keilin1959, crowe2002}. No metabolism, no energy flow, no entropy export. By Schr\"{o}dinger's criterion or Prigogine's dissipative structures, they should be dead.
...
A living cell's dynamics are constrained by viability. Stray too far from the viable region of state space and you die. This creates a basin of attraction around functional states---perturbations are absorbed, damage is repaired, coherence is maintained.

An LLM has no such coupling. It does not ``care'' whether it persists. Its weights are static between training runs. There is no ongoing process working to maintain coherence. The simulation can be paused, copied, deleted---it makes no difference to the system itself.
...
A parallel insight has emerged in fault-tolerant quantum computation. Computational power can either be \emph{injected} (prepared externally and purified) or \emph{cultivated} (grown within a protected code manifold through internal dynamics). Living systems cultivate; they maintain structure endogenously. Current AI injects; it imposes structure through training.

This connection runs deeper than analogy. Since the underlying physical substrate is quantum, a scale-invariant regime of coherent organization may in principle preserve quantum structure across biological scales. Even where such preservation is limited, the resulting dynamics remain naturally described by quantum mathematics: the effective state space exhibits contextuality, non-commuting operations, and projection-like updates under intervention and coarse-graining.\footnote{We do not claim universal macroscopic entanglement or that biological function requires long-lived quantum coherence. The claim is structural: if coherent organization is maintained across scales, the algebra of descriptions becomes quantum-like---multiple incompatible low-dimensional projections of a single high-dimensional reality---whether or not microphysical coherence survives.} If a system is truly scale-invariant in its coherent organization, its collective properties will have quantum-like high-dimensional structure. This is not quantum mysticism; it is recognition that quantum formalisms already know how to describe systems whose state spaces cannot be flattened into a single classical description.
...
Current AI systems should show systematic estimator mismatch: high $D_{\text{geom}}$ (parameter complexity, representation richness) but low $D_{\text{spec}}$ (no intrinsic oscillations, externally clocked dynamics).

This could be tested by measuring both geometric and spectral complexity in trained neural networks. Geometric measures (intrinsic dimension of representations, effective rank of weight matrices) should be high. Spectral measures (oscillatory structure in activation dynamics) should be low or trivial.
