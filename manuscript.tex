...
\begin{enumerate}
    ...
    \item \textbf{Spectral proxy ($D_{\text{spec}}$).} Compute power spectral density via Welch's method. Normalize to probability distribution and compute spectral entropy: $H_S = -\sum_f p(f) \log p(f)$. Convert to effective spectral dimension by exponentiating: $D_{\text{spec}} = e^{H_S}$.

    \item \textbf{Match error.} Compute $\varepsilon = |D_{\text{geom}} - D_{\text{spec}}| / ((D_{\text{geom}} + D_{\text{spec}})/2 + \delta)$ for each window. Report median and interquartile range across windows.
\end{enumerate}
