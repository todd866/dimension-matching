...
\caption{Dimension matching in GMC simulations. (A) Correlation dimension $D_C$ and Fourier dimension $D_F$ both decrease as $\gamma$ increases, tracking each other throughout the subcritical regime. Near the critical point ($\gamma \to \sqrt{2}$), both dimensions approach zero as the measure collapses. (B) Scatter plot of $D_C$ vs.\ $D_F$ across $\gamma$ values, showing approximate agreement (points near the diagonal) in the coherent regime. The solid line shows the theoretical prediction $D^*(\gamma)$; simulated estimates scatter around this curve. The matching reflects the martingale structure that conserves variance across scales.}
\label{fig:matching}
\end{figure}
