...
\noindent\textbf{%
When can we trust a low-dimensional summary of a high-dimensional chaotic system? We show that ``dimension matching''---the agreement between geometric (correlation) and spectral (harmonic) complexity measures---provides a sharp criterion. In multiscale stochastic systems, these independently defined dimensions coincide exactly when a martingale-like balance holds across scales. This balance can be interpreted as a cooperative equilibrium: no scale ``cheats'' the cascade. At a critical threshold, the balance breaks, the dimensions decouple, and low-dimensional projections become unreliable. This framework offers a universal diagnostic for regime collapse applicable to neural dynamics, turbulence, and multi-agent coordination.
}

\vspace{1em}

\begin{abstract}
In multiscale stochastic systems, two fundamentally different notions of complexity—correlation dimension (measuring geometric clumpiness) and harmonic dimension (measuring spectral decay)—can be independently defined. Recent mathematical work on Gaussian multiplicative chaos (GMC) establishes that these dimensions provably coincide below a critical noise threshold, but decouple at the phase transition where the system collapses. We argue this ``dimension matching'' phenomenon reflects a coherence condition: cross-scale consistency maintained by martingale-like conservation laws. When the consistency holds, multiple distinct projections of the high-dimensional dynamics agree; when it fails, low-dimensional summaries become unreliable. We develop a game-theoretic interpretation where dimension matching corresponds to an incentive-compatible multiscale contract—no scale can profitably ``defect'' from the cascade. The breakdown of matching thus serves as a universal diagnostic for regime collapse, applicable to neural dynamics, cellular coordination, and multi-agent systems. Our framework clarifies when coarse-grained observations faithfully represent high-dimensional chaotic structure, and when they must fail.

\textbf{Keywords:} dimension matching; Gaussian multiplicative chaos; multiscale dynamics; phase transitions; coherence; correlation dimension; spectral analysis
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Harmonic dimension} ($D_H$): A spectral measure capturing how Fourier or wavelet coefficients decay with frequency—the ``oscillatory richness'' of the dynamics.
\end{enumerate}

These arise from different mathematical traditions (geometry and probability vs.\ harmonic analysis) and need not agree \emph{a priori}. Yet recent work on Gaussian multiplicative chaos (GMC) establishes a remarkable result: in the subcritical regime, $D_C = D_H$ exactly \citep{garban2023, lin2024}. The dimensions match—until a phase transition, at which point the equivalence breaks and the system collapses.

This paper develops three claims:

\begin{enumerate}
    \item \textbf{Dimension matching as coherence}: The coincidence of $D_C$ and $D_H$ reflects a \emph{cross-scale consistency} condition—what we call coherence. This is not phase-locking (which reduces dimensionality) but the maintenance of structured relationships across many degrees of freedom.
...
The parameter $\gamma \in [0, 2)$ controls the ``strength'' of the chaos:
\begin{itemize}
    \item \textbf{Subcritical} ($\gamma < 2$): The measure is well-defined, multifractal, and captures structured fluctuations at every scale.

    \item \textbf{Critical} ($\gamma = 2$): The measure collapses—mass concentrates on a set of zero Lebesgue measure.

    \item \textbf{Supercritical} ($\gamma > 2$): The naive construction fails entirely; modified definitions are required.
\end{itemize}
...
\caption{Gaussian multiplicative chaos measures at increasing values of the parameter $\gamma$. As $\gamma$ increases toward the critical value of 2, the measure becomes increasingly concentrated (spiky), with mass condensing onto smaller regions. The participation ratio (PR) quantifies this concentration: higher values indicate more uniform distribution, while lower values indicate mass concentration. In the subcritical regime ($\gamma < 2$), the measure remains well-defined; at criticality, it collapses.}
\label{fig:gmc}
\end{figure}
...
For GMC measures $\mu_\gamma$ on the circle, Garban and Vargas defined two dimensions \citep{garban2023}:

\begin{definition}[Correlation Dimension]
The correlation dimension $D_C(\gamma)$ characterizes the decay of correlations:
\begin{equation}
    \mathbb{E}[\mu_\gamma(B(x,r)) \mu_\gamma(B(y,r))] \sim |x-y|^{-\xi(\gamma)} \quad \text{as } |x-y| \to 0
\end{equation}
where the exponent $\xi(\gamma)$ determines $D_C$.
\end{definition}

\begin{definition}[Harmonic Dimension]
The harmonic dimension $D_H(\gamma)$ characterizes spectral decay. Expanding $\mu_\gamma$ in Fourier modes, the power in mode $n$ scales as:
\begin{equation}
    \mathbb{E}[|\hat{\mu}_\gamma(n)|^2] \sim |n|^{-\alpha(\gamma)}
\end{equation}
where $\alpha(\gamma)$ determines $D_H$.
\end{definition}

These dimensions emerge from completely different calculations—one geometric (how ``clumpy'' the measure is), one harmonic (how fast frequencies decay). There is no obvious reason they should agree.

\subsection{The Garban-Vargas Conjecture and Its Resolution}

In 2023, Garban and Vargas conjectured a precise formula relating $D_C$ and $D_H$ for subcritical GMC \citep{garban2023}:

\begin{theorem}[Dimension Matching, \citealt{lin2024}]
\label{thm:matching}
For Gaussian multiplicative chaos with parameter $\gamma \in (0, 2)$:
\begin{equation}
    D_C(\gamma) = D_H(\gamma)
\end{equation}
...
Lin, Qiu, and Tan proved this conjecture by revealing the mechanism: GMC possesses a \emph{martingale structure} across scales \citep{lin2024}. A martingale is a ``fair game''—the expected value at finer scales equals the value at coarser scales, with no systematic drift. For GMC, this means randomness is ``conserved'' across the cascade: each scale contributes fluctuations in a balanced way.

The martingale structure forces an energy conservation law that couples geometric and spectral properties. Because the same conservation law governs both, the dimensions must agree.

\subsection{Mathematical Formalization}

We now provide a more precise formulation of the dimension matching result. For a measure $\mu$ on the circle $\mathbb{T}$, define:

\begin{definition}[Generalized Dimensions]
The \emph{correlation exponent} $\xi$ is defined by the two-point correlation scaling:
\begin{equation}
    \int_{\mathbb{T}} \int_{\mathbb{T}} |x - y|^{-\xi} \, d\mu(x) \, d\mu(y) < \infty
\end{equation}
for the largest $\xi$ for which this integral converges. The correlation dimension is $D_C = 1 - \xi/2$.

The \emph{Fourier dimension} is defined by:
\begin{equation}
    D_F = \limsup_{|n| \to \infty} \frac{-2 \log |\hat{\mu}(n)|}{\log |n|}
\end{equation}
where $\hat{\mu}(n) = \int_{\mathbb{T}} e^{-2\pi i n x} \, d\mu(x)$.
\end{definition}

For GMC measures $\mu_\gamma$, the explicit formulas are known:

\begin{proposition}[Explicit Dimension Formulas for GMC]
For the GMC measure $\mu_\gamma$ with $\gamma \in (0, 2)$:
\begin{equation}
    D_C(\gamma) = D_F(\gamma) = 1 - \frac{\gamma^2}{4}
\end{equation}
Both dimensions decrease quadratically from $1$ (at $\gamma = 0$) to $0$ (at $\gamma = 2$).
\end{proposition}

The key to the proof is the following martingale structure:

\begin{lemma}[Martingale Property]
Let $\mu_\gamma^{(n)}$ denote the GMC measure at resolution $2^{-n}$. Then $\{\mu_\gamma^{(n)}\}_{n \geq 0}$ forms a martingale with respect to the natural filtration generated by the underlying Gaussian field.
\end{lemma}

This martingale property implies that the ``mass'' allocated to each scale is conserved in expectation. The proof of dimension matching proceeds by showing that both $D_C$ and $D_F$ can be computed from the same martingale moments, forcing their equality.

The critical exponent $\gamma = 2$ marks the boundary where the martingale ceases to be uniformly integrable. Beyond this point, mass concentrates on atoms and the dimension matching relationship breaks down.

\subsection{Collapse at Criticality}

At $\gamma = 2$, the martingale balance breaks. Variance explodes, extremes dominate, and the scale-by-scale conservation fails. Concretely:
\begin{itemize}
    \item The GMC measure concentrates on a fractal set of dimension 0 (in a suitable sense)
    \item Correlations become singular
    \item Spectral properties diverge
    \item The relationship $D_C = D_H$ ceases to hold
\end{itemize}
...
\caption{Dimension matching in GMC simulations. (A) Correlation dimension $D_C$ and harmonic dimension $D_H$ both decrease as $\gamma$ increases, tracking each other throughout the subcritical regime. Near the critical point ($\gamma \to 2$), both dimensions approach zero as the measure collapses. (B) Scatter plot of $D_C$ vs.\ $D_H$ across $\gamma$ values, showing approximate agreement (points near the diagonal) in the coherent regime. The matching reflects the martingale structure that conserves ``energy'' across scales.}
\label{fig:matching}
\end{figure}
...
We interpret Theorem~\ref{thm:matching} as a \emph{coherence condition}. The coincidence of $D_C$ and $D_H$ means that different ways of observing the system—geometric vs.\ spectral—yield consistent answers about its complexity. This is nontrivial: for generic stochastic processes, these dimensions can differ arbitrarily.
...
Dimension matching ($D_C = D_H$) corresponds to a cooperative equilibrium where no scale can unilaterally improve its payoff by deviating from the martingale budget.
\end{proposition}
...
Dimension matching provides a testable prediction:
\begin{itemize}
    \item In healthy/awake states, correlation-based and spectral-based complexity measures should agree
    \item Near transitions (seizure, anesthesia, sleep), the dimensions should decouple
    \item The direction of decoupling indicates which scales are ``winning''
\end{itemize}

Figure~\ref{fig:neural} demonstrates this prediction using synthetic EEG data calibrated to different brain states. The key finding: dimension matching (low error between geometric and spectral complexity) is highest in the awake state and degrades significantly during seizure, consistent with the hypothesis that pathological hypersynchrony disrupts the martingale balance across neural scales.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/fig7_neural_analysis.png}
\caption{Dimension matching in simulated neural dynamics across brain states. (A) Example EEG traces showing characteristic patterns: awake (rich, variable), sleep (slower, more coherent), seizure (hypersynchronous), anesthesia (suppressed). (B) Geometric complexity (participation ratio) vs.\ spectral complexity, showing that awake dynamics cluster in the high-complexity region while pathological states show reduced dimensionality. (C) Match error (relative difference between complexity measures) is lowest for awake states and highest for seizure, consistent with the prediction that coherent dynamics maintain dimension matching. (D) Summary showing that seizure and anesthesia exhibit significantly worse dimension matching than awake states ($p < 0.001$, Mann-Whitney U test).}
\label{fig:neural}
\end{figure}
...
\begin{enumerate}
    ...
    \item Its breakdown is a \emph{universal} signature of approaching criticality
    \item The framework provides \emph{operational} metrics: compute $D_C$ and $D_H$ from data, compare
\end{enumerate}

The specific formula $D_C = D_H$ holds exactly for GMC. For other systems, we expect approximate matching in coherent regimes and systematic deviation near transitions.
